...
## %bb.0:
        ...
        .cfi_endproc
                                ## -- End function
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果要透彻理解上述汇编代码，需要先了解以下知识点：</p><ol><li><p>对于rbp、rsp、eax、edi、esi等寄存器，前面的r表示寄存器是64位的，前面的e表示寄存器是32位的。</p></li><li><p>对于pushq、popq、callq、movl、addl等指令，后面的q表示处理的是64位的数据，后面的l表示处理的是32位的数据。</p></li><li><p>栈的内存地址的增长方向是从大到小，也就说栈顶地址比栈底要小。</p></li><li><p>rbp指向栈顶栈帧的底部，rsp指向栈顶栈帧的顶部。</p></li><li><p>函数的返回值通过eax寄存器返回给上层函数。</p></li><li><p>函数的参数通过edi、esi、edx、ecx...等一系列约定好的寄存器来传递。</p></li><li><p>-4(%rbp)表示rbp寄存器中存储的内存地址-4对应的内存单元。</p></li></ol><p>在上述代码中，main()函数调用f_a()函数，f_a()函数调用f_b()函数。我们从f_a()函数开始分析。当刚进入f_a()函数时，栈相关的各个数据如下所示。图中的地址为64位，为了方便表示，用...省略掉了前32位。除此之外，暂时不要纠结main()函数的栈帧里面的具体数据是什么，我们重点放在分析f_a()函数的栈帧上。当f_a()函数的栈帧分析完之后，main()函数的栈帧自然就明了了。</p><p><img src="https://wechatapppro-1252524126.file.myqcloud.com/appnvnpyonz2273/image/b_u_6046034100d02_IEdIiNhL/l2603uw00crb.png" alt="2-3.png" loading="lazy"></p><p>进入f_a()函数之后，最先执行“pushq %rbp”。执行这条指令相当于执行了“subq $8, %rsp”和“movq %rbp, (%rsp)”这两条指令。把rsp寄存器中存储的地址-8，然后将rbp寄存器中存储的值，放入到rsp寄存器存储的地址对应的内存单元中。rbp寄存器中存储的值是main()函数的栈帧底地址，换句话说，也就是把main()函数的栈帧底地址压入栈。pushq指令执行后，栈相关的各个数据如下所示。</p><p><img src="https://wechatapppro-1252524126.file.myqcloud.com/appnvnpyonz2273/image/b_u_6046034100d02_IEdIiNhL/l2604dqx0b1w.png" alt="2-4.png" loading="lazy"></p><p>接下来执行“movq %rsp，%rbp”，将寄存器rsp中的值赋值给rbp。于是，rsp和rbp存储同一个内存地址，也就相当于指向栈中同一个内存单元（栈顶）。</p><p><img src="https://wechatapppro-1252524126.file.myqcloud.com/appnvnpyonz2273/image/b_u_6046034100d02_IEdIiNhL/l2604tg40k9c.png" alt="2-5.png" loading="lazy"></p><p>接下来执行“subq $32, %rsp”，将寄存器rsp中存储的值减去32，给f_a()函数的参数和局部变量预留存储空间。读者可能会说，f_a()函数中参数和局部变量总共有5个int类型的，顶多占20个字节的存储空间呀，为什么预留32个字节？这主要是为了内存对齐。</p><p><img src="https://wechatapppro-1252524126.file.myqcloud.com/appnvnpyonz2273/image/b_u_6046034100d02_IEdIiNhL/l2605azl0plf.png" alt="2-6.png" loading="lazy"></p><p>接下来执行“movl %edi, -4(%rbp)”和“movl %esi, -8(%rbp)”这两条指令，将存储在寄存器edi和esi的参数放入栈中，这样寄存器就可以释放出来以供它用。实际上，参数跟局部变量在本质上是一样的，作用域都在函数内。再接下来的三条指令是计算a1-a2，并将计算结果赋值给a3。这5条指令执行完之后，栈相关的数据如下所示。</p><p><img src="https://wechatapppro-1252524126.file.myqcloud.com/appnvnpyonz2273/image/b_u_6046034100d02_IEdIiNhL/l2605omr09sj.png" alt="2-7.png" loading="lazy"></p><p>接下来调用f_b()函数。先将参数值放入edi和esi寄存器，然后跳转到f_b()函数。我们重点看下callq指令。执行“callq _f_b”相当于执行了“pushq %rip”和“jmp _f_b”两条指令，将指令指针寄存器rip中的值压入栈，然后再跳转去执行f_b()函数的代码。rip寄存器中存储的是callq下一条指令的地址，也就是f_b()函数执行完成，返回到f_a()函数之后，继续往下执行的第一条指令的地址。也就是我们常说的“返回地址”。执行完callq指令之后，栈相关的数据如下图所示。</p><p><img src="https://wechatapppro-1252524126.file.myqcloud.com/appnvnpyonz2273/image/b_u_6046034100d02_IEdIiNhL/l26065sf00uo.png" alt="2-8.png" loading="lazy"></p><p>接下来执行f_b()函数。f_b()函数并没有像f_a()函数那样通过subq指令预留参数和局部变量的存储空间，这是因为f_b()函数是最后一个执行的函数，所以做了一些优化，没有移动rsp指针。f_b()函数中的指令跟f_a()函数中的指令类似，我们就不一条一条分析了。在执行到popq指令之前，栈相关的数据如下图所示。</p><p><img src="https://wechatapppro-1252524126.file.myqcloud.com/appnvnpyonz2273/image/b_u_6046034100d02_IEdIiNhL/l2606j8f0ql0.png" alt="2-9.png" loading="lazy"></p><p>我们重点分析一下f_b()函数中的最后两条指令。</p><p>执行&quot;popq %rbp&quot;指令相当于执行了“movq (%rsp), %rbp”和“addq $8, %rsp”。也就是把栈顶的数据（f_a()函数对应栈帧的帧底地址）取出放入rbp寄存器中。执行完popq指令之后，rsp和rbp分别指向f_a函数栈帧的底和顶。相当于将f_b()函数的栈帧弹出栈。具体如下图所示。</p><p><img src="https://wechatapppro-1252524126.file.myqcloud.com/appnvnpyonz2273/image/b_u_6046034100d02_IEdIiNhL/l26070d90hk5.png" alt="2-10.png" loading="lazy"></p><p>执行“retq”指令相当于执行了“popq %rip”指令，也就是将栈顶的数据（f_a()函数的“返回地址”）取出放入%rip。这样，CPU就跳转去执行f_a()函数中callq指令之后的指令了。执行完retq指令之后，栈相关数据如下图所示。</p><p><img src="https://wechatapppro-1252524126.file.myqcloud.com/appnvnpyonz2273/image/b_u_6046034100d02_IEdIiNhL/l2607csz0lgq.png" alt="2-11.png" loading="lazy"></p><p>回到f_a()函数之后，执行callq后面的4条指令，将a2-a1的值只写a5中。于是，f_a()函数也就完全执行完了。接下来是移除f_a()函数的栈帧的操作。执行“addq $32, %rsp”指令，将rsp指向如图所示位置。</p><p><img src="https://wechatapppro-1252524126.file.myqcloud.com/appnvnpyonz2273/image/b_u_6046034100d02_IEdIiNhL/l2607n780bd4.png" alt="2-12.png" loading="lazy"></p><p>执行“popq %rbp”指令，将栈顶的数据（main()函数的栈帧的帧底地址）赋值给rbp。此时，rsp和rbp指向main()函数的栈帧的帧顶和帧底。相当于将f_a()函数的栈帧弹出栈。</p><p><img src="https://wechatapppro-1252524126.file.myqcloud.com/appnvnpyonz2273/image/b_u_6046034100d02_IEdIiNhL/l26082fu0u8i.png" alt="2-13.png" loading="lazy"></p><p>执行“retq”指令，相当于执行“popq %rip”指令，将main()函数的返回地址赋值给rip，CPU跳转执行main()函数中callq之后的指令。</p><p>详细分析了代码的执行流程之后，对于前面的3个问题，我们再总结回答一下。</p><p>1）栈帧中除了保存局部变量之外，还保存哪些其他数据？</p><p>栈帧中依次保存：前一个栈帧的帧底地址，参数，局部变量，返回地址。保存前一个栈帧的帧底地址的目的是，方便当前函数执行完成之后，rbp指针重新指向前一个栈帧的栈底。保存返回地址的目的是，方便当前函数执行完成之后，返回到上层函数继续执行。</p><p>2）上一节讲到的SP寄存器和BP寄存器具体用在哪里？</p><p>SP寄存器存储栈顶地址，方便将新数据压入栈。BP寄存器存储的是当前栈帧帧底地址，方便基于这个地址的偏移来访问参数、局部变量。</p><p>3）CPU在执行完某个函数之后，如何知道应该回到上层函数的哪处再继续执行？</p><p>在通过callq指令调用函数时，callq指令会将当前的rip寄存器中的内容（callq指令的下一条指令的内存地址，即返回地址）存储在栈帧的最顶端。当被调用的函数执行完之后，被调用函数的栈帧释放，最后调用retq指令（相当于popq %rip），将返回地址重新赋值给rip，CPU就可以从函数中callq指令的下一条指令继续执行了。</p><h2 id="八、课后思考题" tabindex="-1"><a class="header-anchor" href="#八、课后思考题" aria-hidden="true">#</a> <strong>八、课后思考题</strong></h2><p>1）你熟悉的编程语言中，哪些是静态类型语言？哪些是动态类型语言？</p><p>2）我们提到，goto语法在编程语言中被废弃，但为什么汇编语言或机器指令中还保留类似goto语句的jmp指令呢？</p>`,60);function v(m,b){const e=c("ExternalLinkIcon");return p(),l("div",null,[d,s("p",null,[n("对于绝大多数Java程序员来说，即便是要了解Java语言的底层原理或者优化代码性能，也只需要研究到字节码这一层就足够了，完全不需要深入到汇编代码层面。所以，对于如何获取Java代码对应的汇编代码，这不是专栏讲解的重点。不过，为了满足一些朋友的好奇心，我编写了查看Java代码对应汇编代码的操作文档，放到了资料中（"),s("a",r,[n("点击此处"),t(e)]),n("查看），感兴趣的可以看一下。")]),u])}const g=i(o,[["render",v],["__file","2.基础语法.html.vue"]]);export{g as default};
